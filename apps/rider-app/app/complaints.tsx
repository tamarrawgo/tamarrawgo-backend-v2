...
export default function ComplaintsScreen() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      await api.post('/support/complaints', {
        ...
        reportedUserId: selectedTrip?.passengerId,
      ...
      <View style={styles.container}>
        ...
        {loadingTrips ? (
          ...
          <ScrollView contentContainerStyle={styles.tripList}>
            {trips.map((trip) => {
              const passengerName = `${trip.passenger?.firstName ?? ''} ${trip.passenger?.lastName ?? ''}`.trim();
              ...
              return (
                <TouchableOpacity key={trip.id} style={styles.tripCard} onPress={() => setSelectedTrip(trip)}>
                  ...
                  <View style={styles.tripInfo}>
                    <Text style={styles.tripName}>{passengerName || 'Unknown Passenger'}</Text>
  ...
  const passengerName = `${selectedTrip.passenger?.firstName ?? ''} ${selectedTrip.passenger?.lastName ?? ''}`.trim();
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.title}>Report Passenger</Text>
        <View style={{ width: 24 }} />
      </View>
      ...
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
        <ScrollView contentContainerStyle={styles.content}>
          <View style={styles.reportedCard}>
            ...
            <View>
              <Text style={styles.reportedName}>{passengerName}</Text>
